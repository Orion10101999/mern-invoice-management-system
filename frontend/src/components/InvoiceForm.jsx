...
const InvoiceForm = ({ invoice, setInvoice }) => {
  const [invoiceNumber, setInvoiceNumber] = useState(e?.invoiceNumber || '');
  ...
  const [status, setStatus] = useState(e?.status || 'unpaid');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (invoice) {
        await axios.put(`/api/invoices/${invoice._id}`, invoiceData, config);
      } else {
        await axios.post('/api/invoices', invoiceData, config);
      }
      setInvoice(null);
    } catch (error) {
      console.error(error);
